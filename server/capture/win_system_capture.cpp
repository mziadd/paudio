// Windows 11: capture system speaker output using WASAPI loopback (float32, 48 kHz).
// Most Win 11 systems run the speaker mix at 48 kHz float32 by default — we pass
...
void WinSystemCapture::captureThread() {
    ...
    if (FAILED(hr)) { last_error_ = "no device enumerator"; goto cleanup; }

    hr = enumerator->GetDefaultAudioEndpoint(eRender, eConsole, &device);
    if (FAILED(hr)) { last_error_ = "no default audio output"; goto cleanup; }

    hr = device->Activate(__uuidof(IAudioClient), CLSCTX_ALL, nullptr,
                          reinterpret_cast<void**>(&client));
    if (FAILED(hr)) { last_error_ = "activate audio client failed"; goto cleanup; }

    hr = client->GetMixFormat(&mixFormat);
    if (FAILED(hr)) { last_error_ = "GetMixFormat failed"; goto cleanup; }

    // AUDCLNT_STREAMFLAGS_LOOPBACK is the magic flag — taps the speaker mix.
    hr = client->Initialize(AUDCLNT_SHAREMODE_SHARED, AUDCLNT_STREAMFLAGS_LOOPBACK,
                            10000000 /* 1 second buffer */, 0, mixFormat, nullptr);
    if (FAILED(hr)) { last_error_ = "IAudioClient::Initialize failed"; goto cleanup; }

    hr = client->GetService(__uuidof(IAudioCaptureClient),
                            reinterpret_cast<void**>(&capture));
    if (FAILED(hr)) { last_error_ = "GetService failed"; goto cleanup; }

    hr = client->Start();
    if (FAILED(hr)) { last_error_ = "Start failed"; goto cleanup; }

    {
        ...
        const bool srcIsFloat    = (mixFormat->wFormatTag == WAVE_FORMAT_IEEE_FLOAT) || bits == 32;

        if (srcRate != kSampleRateHz) {
            std::cerr << "  warning: speaker mix is " << srcRate
                      << " Hz, expected " << kSampleRateHz
                      << " Hz — set speakers to 48000 Hz in Sound Properties.\n";
        }

        std::vector<float> pending;
        pending.reserve(static_cast<std::size_t>(kChunkSamplesPerChannel) * kChannelCount * 4);
