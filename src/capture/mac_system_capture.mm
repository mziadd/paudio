// macOS 15+: ScreenCaptureKit system audio capture (float32, 48 kHz).
//
// SCK only delivers audio when the stream also produces video — so we attach
// a 2x2, 30 fps video output and discard the frames.
//
// Hot-path goals:
//   * No allocations once running. The pending buffer is pre-reserved and
//     reused; samples are appended with memcpy where possible.
...
// Headroom: capture callback may deliver a few thousand samples at once.
...
struct CaptureState {
    ...
    std::mutex         mutex;
    // Interleaved stereo float samples. We never erase from the front in the
    // hot path; head_ tracks how many have already been flushed.
...
// Push every full chunk (kChunkSamplesPerChannel * 2 floats) to the callback.
void flushChunks(CaptureState* state) {
    ...
    while (state->pending.size() - state->head >= chunkLen) {
        ...
        std::memcpy(chunk.data(),
                    state->pending.data() + state->head,
                    chunkLen * sizeof(float));
        state->head += chunkLen;
        if (state->callback) state->callback(chunk);
    }

    // Compact occasionally so pending doesn't grow without bound.
    if (state->head >= kPendingCompactThreshold) {
        const std::size_t left = state->pending.size() - state->head;
        if (left > 0) {
            std::memmove(state->pending.data(),
                         state->pending.data() + state->head,
                         left * sizeof(float));
        }
...
// Append a CMSampleBuffer of float samples as interleaved stereo into `pending`.
// SCK delivers either interleaved (single buffer) or planar (one per channel).
void appendSampleBuffer(CaptureState* state, CMSampleBufferRef sample) {
    ...
    if (!asbd || !(asbd->mFormatFlags & kAudioFormatFlagIsFloat)) return;

    std::size_t listSize = 0;
    if (CMSampleBufferGetAudioBufferListWithRetainedBlockBuffer(
            sample, &listSize, nullptr, 0, nullptr, nullptr, 0, nullptr) != noErr) return;
    ...
    if (st != noErr) { std::free(list); return; }

    const int  channels       = asbd->mChannelsPerFrame > 0
                                  ? static_cast<int>(asbd->mChannelsPerFrame) : 2;
    ...
    if (nonInterleaved && list->mNumberBuffers >= 1) {
        ...
        const std::size_t frames =
            list->mBuffers[0].mDataByteSize / (sizeof(float) * static_cast<std::size_t>(channels));
        ...
        if (channels == 2) {
            // Fast path: identical stride → one memcpy.
            ...
            // 3+ channels: take L/R and ignore the rest.
            for (std::size_t i = 0; i < frames; ++i) {
...
// Receives sample buffers on a background queue (QOS_USER_INTERACTIVE).
...
- (void)stream:(SCStream*)stream
    ...
    (void)stream;
    if (!_state || type != SCStreamOutputTypeAudio) return;
...
bool MacSystemCapture::start(ChunkCallback on_chunk) {
    if (running_)  { last_error_ = "already running";   return false; }
    if (!on_chunk) { last_error_ = "callback required"; return false; }
    ...
    if (@available(macOS 15.0, *)) {
        ...
        [SCShareableContent getShareableContentExcludingDesktopWindows:NO
            ...
            if (err || content.displays.count == 0) {
                errMsg = err.localizedDescription ?: @"no display available";
            ...
            // Tiny screen stream — required for SCK to deliver audio at all.
            cfg.width                = 2;
            cfg.height               = 2;
            ...
            StreamReceiver* receiver = [[StreamReceiver alloc] init];
        ...
        if (!ok) {
            last_error_  = errMsg ? [errMsg UTF8String] : "ScreenCaptureKit failed to start";
